...
针对MLC问题的方法通常都含有大量的问题转换(`Problem Transformation`)，假设情绪集合为 $\mathcal{L}=\left\{l\_{1},l\_{2},\cdots,l\_{k}\right\}$，主要有如下三种：
...
设BR变换的分类器为 $C\_{j}^{B}(j \in[1 \cdots k])$。其中，分类器 $C_{j}^{B}$ 只用作生成$y\_i$。换言之，$C\_j^B$ 需要学习到概率分布 $\mathcal{P}\left(y\_{i} | X\right)$，$Y^b$ 是通过所有的 k 个分类器共同得到的。
...
$$
将前向和后向的隐层状态分别记作 $\overrightarrow{h}_{t}^{D}$和 $\overleftarrow{h}_{t}^{D}$ ，将其拼接后输入全连接层，有
$$
...
$$
$Y_i^{te}$代表数据集中的真实的标签集合，$\hat{Y}_{i}^{t e}$是模型预测的标签集合。
