...
作者选择`BERT`来将关键词分配给不同的句子并预测他们的位置。如下图所示，`prompt` $x$ 和无序关键词组 $m$ 后由双向`self attention`编码。关键词组的分配 $\boldsymbol{m}^{\prime}=\{w_{j}}$ 将以自回归的方式进行，他们在句子 $\boldsymbol{s}=\left\{s_{j}\right\}$ 的位置预测被视作一个序列标注问题。
